...
namespace VMP_CNR
{
    public static class RemoteEventKeyExtension
    {
        public static bool CheckRemoteEventKey(this Player player, string key)
        {
            if (key == null || key == "") return false;
            DbPlayer dbPlayer = player.GetPlayer();

            if (dbPlayer == null || !dbPlayer.IsValid(true)) return false;

            if (key != dbPlayer.RemoteHashKey)
            {
                Logger.Debug("Incorrect RemoteHashKey from " + dbPlayer.GetName() + " (" + key + ")");
                return false;
            }
            return true;
        }
