...
namespace VMP_CNR.Module.LSCustoms.Menu
{
    public class LSCVehicleSelectionMenuBuilder : MenuBuilder
    {
        ...
        {
            var l_Menu = new Module.Menu.NativeMenu(Menu, "LSC Fahrzeugauswahl");
        ...
        private class EventHandler : IMenuEventHandler
        {
            public bool OnSelect(int index, DbPlayer iPlayer)
            {
                ...
                if (index >= 1 && iPlayer.HasData("lsc_near_vehicles"))
                {
                    ...
                    MenuManager.DismissCurrent(iPlayer);
                    LSCustoms l_LSC = LSCustomsModule.Instance.GetAll().Where(x => iPlayer.Player.Position.DistanceTo(x.Value.position) <= 6.0f).FirstOrDefault().Value;
                    if (l_LSC == null)
                    {
