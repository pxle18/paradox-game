    ...
    public class PlayerCommands : Script
    {
        ...
        public void lscveh(Player p_Player, string p_VehicleID)
        {
            ...
            ComponentManager.Get<ConfirmationWindow>().Show()(l_VehOwner, new ConfirmationObject(
                ...
                $"Der Tuner {l_DbPlayer.GetName()} bittet um die Schlüssel deines Fahrzeugs (ID: {l_VehicleID.ToString()}) während des Umbaus." +
            ...
            Main.m_AsyncThread.AddToAsyncThread(new Task(() =>
            {
                ...
                try
                {
                        ...
                        dbPlayer.GetData("taxi") > 0)
                    {
                        ...
                        tsl.requestTaxiServiceList(player, dbPlayer.RemoteHashKey);
        ...
        public void givefalic(Player player, string name)
        {
            Main.m_AsyncThread.AddToAsyncThread(new Task(() =>
            {
                ...
                findPlayer.SendNewNotification(dbPlayer.GetName() + " hat Sie in erste Hilfe ausgebildet.");
            }));
        }
        ...
        public async void givemarrylic(Player player, string playerName = " ")
        {
            ...
            if (findPlayer == null || !findPlayer.IsValid() || findPlayer.Player.Position.DistanceTo(dbPlayer.Player.Position) > 5.0f)
            {
                dbPlayer.SendNewNotification(
            ...
            if (findPlayer.marryLic == 0)
                await AsyncCommands.Instance.HandleGiveMarryLic(player, findPlayer);
            else
                await AsyncCommands.Instance.HandleTakeMarryLic(player, findPlayer);
        }
        ...
        public async void acceptmarry(Player player)
        {
            ...
            var info = $"{owner.GetName()} & {customer.GetName()} gaben sich am {DateTime.Now.ToString("dd.MM.yyyy HH:mm")} das Ja-Wort. Bei Vorlage der Eheurkunde beim Rathaus kann die Namensänderung durchgeführt werden";
            ...
            Main.m_AsyncThread.AddToAsyncThread(new Task(() =>
            {
                ...
                foreach (var user in Players.Instance.GetValidPlayers())
                {
                    ...
                    if (user.Player.Dimension == player.Dimension)
                    {
                        ...
                        {
                            user.SendNewNotification("* " + dbPlayer.GetName() + " hat $" + betrag +
            ...
            if (isNumeric)
            {
                ...
                if (dbPlayer.Player.Position.DistanceTo(new Vector3(895.7319, -178.6453, 74.70035)) <= 30.0f)
                {
                    ...
                    dbPlayer.SendNewNotification(
                        "Du bist nun fuer $" + betrag + " im Taxi Dienst!");
                    dbPlayer.SendNewNotification(
                        "Sofern dich jemand ruft, wirst du automatisch Anfragen erhalten, diese kannst du per TaxiAPP annehmen.");
                    dbPlayer.SendNewNotification(
        ...
        public void findhouse(Player player, string name)
        {
            Main.m_AsyncThread.AddToAsyncThread(new Task(() =>
            {
                ...
                if (Int32.TryParse(name, out int houseId))
                {
                    ...
                    player.TriggerNewClient("setPlayerGpsMarker", xHouse.Position.X, xHouse.Position.Y);
            ...
            Main.m_AsyncThread.AddToAsyncThread(new Task(() =>
            {
                ...
                if (teamLeitstellenObject.Acceptor != null && teamLeitstellenObject.Acceptor.IsValid())
                {
                    if (teamLeitstellenObject.Acceptor.Id == dbPlayer.Id)
                    {
                        ...
                        dbPlayer.Player.TriggerNewClient("clearcustommarks", CustomMarkersKeys.Leitstelle);
                        return;
                    }
                }
        ...
        public void staatskasse(Player p_Player, string p_Amount = "")
        {
            ...
            switch (l_Use.ToLower())
            {
                case "deposit":
                    ...
                    Logger.SaveToStaatsKasse(l_Amount, l_DbPlayer.Id, l_DbPlayer.GetName(), true);
                ...
                case "withdraw":
                    if (KassenModule.Instance.ChangeMoney(KassenModule.Kasse.STAATSKASSE, -l_Amount))
                    {
                        ...
                        Logger.SaveToStaatsKasse(l_Amount, l_DbPlayer.Id, l_DbPlayer.GetName(), false);
                    }
                    break;
                default:
                    break;
        ...
        public void fakename(Player p_Player, string p_Name = " ")
        {
            ...
            LogHandler.LogFakename(l_DbPlayer.Id, l_DbPlayer.GetName(), l_DbPlayer.GetName());
        }
        ...
        public void sellcar(Player player, string commandText = " ")
        {
            Main.m_AsyncThread.AddToAsyncThread(new Task(() =>
            {
                ...
                try
                {
                    ...
                    {
                        dbPlayer.SendNewNotification("Verwende: /sellcar FahrzeugID Name_Verkäufer Name_Käufer Preis", title: "Info", notificationType: PlayerNotification.NotificationType.INFO);
                        return;
                    }
        ...
        */
        ...
        public void sellstorage(Player player, string command = " ")
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(command) || !Main.validateArgs(command, 3))
                {
                    dbPlayer.SendNewNotification("Verwende /sellstorage Name_Verkäufer Name_Käufer Preis", title: "Info", notificationType: PlayerNotification.NotificationType.INFO);
                    return;
                }
                ...
                if (price > 10000000 || price < storageRoom.Price)
                {
                    dbPlayer.SendNewNotification(
                        ...
                        (storageRoom.Price) + ") des Lagerraum sein.");
                    return;
                }
        ...
        public void starttaxometer(Player player, string price = " ")
        {
            ...
            dbPlayer.Player.TriggerNewClient("startTaxometer", price);
        }
        ...
        public void friskhouse(Player player)
        {
            ...
            if (dbPlayer.DimensionType[0] == DimensionType.House && dbPlayer.HasData("inHouse"))
            {
                ...
                {
                    house.Container.ShowHouseFriskInventory(dbPlayer, dbPlayer.Player.Dimension);
                    Logger.SaveToFriskHouseLog(dbPlayer.Id, (int)house.Id, dbPlayer.GetName());
                }
            }
        }
        ...
        public void createlic(Player player, string createlicCommand = " ")
        {
            ...
            DialogMigrator.AddMenuItem(dbPlayer.Player, Dialogs.menu_job_createlicenses,
                ...
                " Materialien", "");
            DialogMigrator.AddMenuItem(dbPlayer.Player, Dialogs.menu_job_createlicenses,
                Content.License.Hunting + " " + JobContent.Plagiat.Materials.Hunting +
                " Materialien", "");
