...
namespace VMP_CNR.Module.Players.Seat
{
    public class PlayerSeatModule : SqlModule<PlayerSeatModule, PlayerSeat, int>
    {
        ...
        [RemoteEvent]
        public void Interact(Player player, int model, Vector3 position, Vector3 rotation, NetHandle seatNetHandle, string key)
        {
            // Not used anymore
            ...
            else
            {
                ...
                var heading = -1f;
                for (int i = 1, length = occupiedInts.Length; i < length; i++)
                {
                ...
                if (destination == null || freeIndex == 0) return; //No free seat
        ...
        [RemoteEvent]
        public void CancelInteract(Player player, string key)
        {
            /* NOT USED ANYMORE
            if (!player.CheckRemoteEventKey(key)) return;
            DbPlayer dbPlayer = player.GetPlayer();
        ...
        private void RemovePlayerSeatReservation(DbPlayer dbPlayer)
        {
            ...
            dbPlayer.CurrentSeat = 0;
            dbPlayer.CurrentSeatIndex = 0;
        }
