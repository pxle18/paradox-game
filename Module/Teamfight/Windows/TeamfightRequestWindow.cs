...
namespace VMP_CNR.Module.Teamfight.Windows
{
    public class TeamfightRequestWindow : Window<Func<DbPlayer, uint, int, uint, string, bool, bool>>
    {
        ...
        [RemoteEvent]
        public void acceptTeamfight(Player player, uint attackerTeam, int money, string key)
        {
        ...
        [RemoteEvent]
        public void declineTeamfight(Player player, uint attackerTeam, string key)
        {
            if (!player.CheckRemoteEventKey(key)) return;
            DbPlayer dbPlayer = player.GetPlayer();
            if (dbPlayer == null) return;
