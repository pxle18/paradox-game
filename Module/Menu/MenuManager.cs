...
namespace VMP_CNR.Module.Menu
{
    ...
    public class MenuManager
    {
        ...
        public void OnSelect(PlayerMenu menu, int index, DbPlayer dbPlayer)
        {
            if (menuEventHandlers.ContainsKey(menu))
            {
                if (menuEventHandlers[menu].OnSelect(index, dbPlayer))
                {
                    DismissMenu(dbPlayer.Player, (uint) menu);
                }
            }
        }

        public static void DismissCurrent(DbPlayer dbPlayer)
        {
            dbPlayer.Player.TriggerNewClient("componentServerEvent", "NativeMenu", "hide");
        }
