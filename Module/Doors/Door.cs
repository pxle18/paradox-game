        ...
        public void Refresh()
        {
            foreach(DbPlayer player in Players.Players.Instance.GetValidPlayers())
            {
                if(player.Player.Position.DistanceTo(Position) < Range*5) // Update for players in Range * 3
                {
                    ...
                    if (Model >= 0)
                        player.Player.TriggerNewClient("setStateOfClosestDoorOfType", (uint)Model, Position.X, Position.Y, Position.Z, Locked, 0, false);
                    else
                        player.Player.TriggerNewClient("setStateOfClosestDoorOfType", (int)Model, Position.X, Position.Y, Position.Z, Locked, 0, false);
                }
            }
        }
    }
