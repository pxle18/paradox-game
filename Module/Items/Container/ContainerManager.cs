...
namespace VMP_CNR.Module.Items
{
    public static class ContainerManager
    {
        ...
        public static async Task<Container> LoadContainerAsync(uint id, ContainerTypes type, int tempDefaultSlots = 0, int tempDefaultWeight = 0)
        {
            if (id == 0)
            {
                return CreateContainer(id, type, tempDefaultSlots, tempDefaultWeight);
        ...
        public static bool IsItemRestrictedForContainer(this Container container, ItemModel item, DbPlayer dbPlayer = null)
        {
            ...
            if (container.Type == ContainerTypes.RAFFINERY || container.Type == ContainerTypes.TeamOrder || 
                container.Type == ContainerTypes.METHLABORATORYOUTPUT || container.Type == ContainerTypes.CANNABISLABORATORYOUTPUT ||
            ...
            if (container.Type == ContainerTypes.BLACKMONEYINVENTORY)
                if (SchwarzgeldModule.SchwarzgeldId != item.Id)
                    return true;
            if (container.Type == ContainerTypes.HEROINLABORATORYINPUT)
                if (!HeroinlaboratoryModule.RessourceItemIds.Contains(item.Id))
                    return true;
            if (container.Type == ContainerTypes.HEROINLABORATORYOUTPUT)
                if (!HeroinlaboratoryModule.EndProductItemIds.Contains(item.Id))
                    return true;
            if (container.Type == ContainerTypes.HEROINLABORATORYFUEL)
                if (HeroinlaboratoryModule.FuelItemId != item.Id)
                    return true;
        ...
        public static ClientContainerObject ConvertForClient(this Container container, int sendId, string optionalname = "", int money = 0, int blackmoney = 0, bool playerActions = false)
        {
            ...
            clientContainerObject.PlayerInventory = playerActions;
        ...
        private static bool AddContainerItem(this Container container, ItemModel model, int amount, Dictionary<string, dynamic> data = null, int slot = -1, bool disablesave = false)
        {
            ...
            if (model.ItemModelType == ItemModelTypes.Radio)
            {
                if (container.Type == ContainerTypes.PLAYER)
                {
                    DbPlayer dbPlayer = container.GetPlayerByContainer();
                    if (dbPlayer != null && dbPlayer.IsValid())
                    {
            ...
            if (model.ItemModelType == ItemModelTypes.Radio)
            {
                if (container.Type == ContainerTypes.PLAYER)
                {
                    ...
                    if (dbPlayer != null && dbPlayer.IsValid())
                    {
                        VoiceModule.Instance.ChangeFrequenz(dbPlayer, 0f);
                        dbPlayer.UpdateApps();
                    }
                }
            }
        ...
        public static void CheckFunkDisabling(this Container container)
        {
            if (container.Type == ContainerTypes.PLAYER)
            {
                ...
                if (dbPlayer != null)
                {
                    if (!VoiceModule.Instance.hasPlayerRadio(dbPlayer)) Voice.VoiceModule.Instance.turnOffFunk(dbPlayer);
                    return;
                }
            }
        }
