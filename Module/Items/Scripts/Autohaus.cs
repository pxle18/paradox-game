...
namespace VMP_CNR.Module.Items.Scripts
{
    ...
    {
        public static bool UseCarsell(DbPlayer dbPlayer, ItemModel ItemData, Item item)
        {
            ...
            if(dbPlayer.TeamId == teamId) // Read Only
            {
                ...
                dbPlayer.SendNewNotification($"({contractId}) Lieferschein {vehName} | Kunde {customerName}");
                return false;
            }
            else
            {
                ...
                if (contractId == 0)
                {
                    dbPlayer.SendNewNotification($"Fehler, bitte im Support melden!");
                    return false;
                }
                ...
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                if(dbPlayer.Id != reader.GetUInt32("player_id"))
                                {
                                    dbPlayer.SendNewNotification($"Dieser Lieferschein kann nur vom Besitzer eingelöst werden!");
                                    return false;
                                }
                                ...
                                dbPlayer.SendNewNotification($"Ihr Fahrzeug wurde erfolgreich ausgeliefert (Pillbox Garage)!");
                                return true;
                            }
                        }
                    }
