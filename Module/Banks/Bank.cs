...
namespace VMP_CNR.Module.Banks
{
    public class Bank : Loadable<uint>
    {
        ...
        public bool ActivatedToBreak { get; set; }
        ...
        public Bank(MySqlDataReader reader) : base(reader)
        {
            ...
            if (Type == 2)
            {
                Main.ServerBlips.Add(Blips.Create(Position, "", 207, 1.0f, true, 25));
            }
            else if (Type == 3)
            {
                Main.ServerBlips.Add(Blips.Create(Position, "", 207, 1.0f, true, 46));
            }

            LastBreaked = reader.GetDateTime("last_breaked");
        }
        ...
        public void UpdateLastBreaked()
        {
            LastBreaked = DateTime.Now;
            string query = $"UPDATE `bank` SET last_breaked = '{LastBreaked}' WHERE id = {Id};";
            MySQLHandler.ExecuteAsync(query);
        }
