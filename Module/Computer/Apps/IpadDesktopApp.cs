...
namespace VMP_CNR.Module.Computer.Apps
{
    public class IpadDesktopApp : App<Func<DbPlayer, List<ComputerAppClientObject>, bool>>
    {
        ...
        [RemoteEvent]
        public void requestIpadApp(Player player, string key)
        {
            ...
            {
                if (dbPlayer.CanAccessComputerApp(kvp.Value) && kvp.Value.Type == ComputerTypes.AdminTablet) computerAppClientObjects.Add(new ComputerAppClientObject(kvp.Value));
            }

            TriggerNewClient(player, "responseIpadApps", NAPI.Util.ToJson(computerAppClientObjects));
        }
    }
}
