...
namespace VMP_CNR.Module.Business.Raffinery
{
    ...
    {
        [RemoteEvent]
        public void LoadIntoVehicle(Player player, string returnstring, string key)
        {
            ...
            Task.Run(async () =>
            {
                ...
                {
                    if (rafinnery.GetOwnedBusiness() == dbPlayer.GetActiveBusiness() && dbPlayer.GetActiveBusinessMember() != null && dbPlayer.GetActiveBusinessMember().Raffinery) // Member of business and has rights
                    {
