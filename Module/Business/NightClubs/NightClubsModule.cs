    ...
    public class NightClubsModule : Module<NightClubsModule>
    {
        ...
        public override bool OnColShapeEvent(DbPlayer dbPlayer, ColShape colShape, ColShapeState colShapeState)
        {
            if (colShape.HasData("nightclubId"))
            {
                ...
                if (colShapeState == ColShapeState.Enter)
                {
                    ...
                    dbPlayer.Player.TriggerNewClient("sendInfocard", nightClub.Name, "red", "nightclubs.jpg", 20000, JsonConvert.SerializeObject(temp));
                    return true;
                }
        ...
        public override bool OnKeyPressed(DbPlayer dbPlayer, Key key)
        {
            if (key == Key.E)
            {
                ...
                {
                    NightClub nightClub = NightClubModule.Instance.Get(dbPlayer.Player.Dimension);
                    if (nightClub != null && dbPlayer.IsMemberOfBusiness() && dbPlayer.GetActiveBusiness().BusinessBranch.NightClubId == nightClub.Id)
                    {
                ...
                if (dbPlayer.HasData("nightclubId"))
                {
                    ...
                    if (nightClub != null)
                    {
                        if (nightClub.Position.DistanceTo(dbPlayer.Player.Position) < 2.0f && !dbPlayer.RageExtension.IsInVehicle)
                        {
                            if (!nightClub.IsOwnedByBusines())
                            {
                ...
                if (dbPlayer.Player.Dimension != 0 && dbPlayer.Player.Position.DistanceTo(InteriorEnterPosition) < 2.0f && !dbPlayer.RageExtension.IsInVehicle)
                {
                    NightClub nightClub = NightClubModule.Instance.Get(dbPlayer.Player.Dimension);
            ...
            if (key == Key.L)
            {
                if (dbPlayer.HasData("nightclubId"))
                {
                    ...
                    if (nightClub != null)
                    {
                        ...
                        {
                            if (nightClub.IsOwnedByBusines() && dbPlayer.IsMemberOfBusiness() && dbPlayer.GetActiveBusiness().BusinessBranch.NightClubId == nightClub.Id)
                            {
                                if (!nightClub.Locked)
                                {
