...
namespace VMP_CNR.Module.Laboratories.Windows
{
    public class CannabislaboratoryStartWindow : Window<Func<DbPlayer, Cannabislaboratory, bool>>
    {
        ...
        [RemoteEvent]
        public void saveCannabisLabor(Player client, float temperature, float pressure, float stirring, float amount, string key)
        {
        ...
        [RemoteEvent]
        public void toggleCannabisLabor(Player client, bool result, string key)
        {
            if (!client.CheckRemoteEventKey(key)) return;
            DbPlayer dbPlayer = client.GetPlayer();
