...
namespace VMP_CNR.Module.Laboratories.Windows
{
    public class MethlaboratoryStartWindow : Window<Func<DbPlayer, Methlaboratory, bool>>
    {
        ...
        [RemoteEvent]
        public void saveMethLabor(Player client, float temperature, float pressure, float stirring, float amount, string key)
        {
        ...
        [RemoteEvent]
        public void toggleMethLabor(Player client, bool result, string key)
        {
            if (!client.CheckRemoteEventKey(key)) return;
            DbPlayer dbPlayer = client.GetPlayer();
