...
namespace VMP_CNR.Module.Carsell.Menu
{
    public class CarsellDeliverCustomerMenuBuilder : MenuBuilder
    {
        ...
        {
            var l_Menu = new Module.Menu.NativeMenu(Menu, "Bestellung abschliessen");
        ...
        private class EventHandler : IMenuEventHandler
        {
            public bool OnSelect(int index, DbPlayer dbPlayer)
            {
                ...
                // Show finished orders from team
                foreach (DeliveryOrder deliveryOrder in JobCarsellFactionModule.Instance.DeliverableOrderList.ToList().Where(o => o.TeamId == dbPlayer.TeamId))
                {
                    if (idx == index)
                    {
                        VehicleData vehData = VehicleDataModule.Instance.GetDataById(deliveryOrder.VehicleDataId);
                        ...
                        if(!dbPlayer.Container.CanInventoryItemAdded(641))
                        {
                            dbPlayer.SendNewNotification("Sie haben keinen Platz für einen Kaufvertrag!");
                            return false;
                        }
                        ...
                        var info = $"Besitzer: {PlayerNameModule.Instance.Get(deliveryOrder.PlayerId).Name} Fahrzeug: {(vehData.mod_car_name.Length <= 0 ? vehData.Model : vehData.mod_car_name)} ({id}) am {DateTime.Now.ToString("dd.MM.yyyy HH:mm")}. VK von {dbPlayer.GetName()}";

                        dbPlayer.Container.AddItem(641, 1, new Dictionary<string, dynamic>() { { "Info", info }, { "vehicleId", id } });

                        dbPlayer.SendNewNotification($"Sie haben {(vehData.mod_car_name.Length <= 0 ? vehData.Model : vehData.mod_car_name)} von {PlayerNameModule.Instance.Get(deliveryOrder.PlayerId).Name} für die Liefergarage freigegeben");
