...
namespace VMP_CNR.Module.JobFactions.Carsell
{
    ...
    {
        [RemoteEvent]
        public void JobCarsellSetCarColor(Player player, string returnstring, string key)
        {
        ...
        [RemoteEvent]
        public void JobCarsellCreateOrder(Player player, string returnstring, string key)
        {
            if (!player.CheckRemoteEventKey(key)) return;
            DbPlayer dbPlayer = player.GetPlayer();
            if (dbPlayer == null || !dbPlayer.IsValid()) return;
        ...
        [RemoteEvent]
        public void JobCarsellFinishOrder(Player player, string returnstring, string key)
        {
            if (!player.CheckRemoteEventKey(key)) return;
            DbPlayer dbPlayer = player.GetPlayer();
            if (dbPlayer == null || !dbPlayer.IsValid()) return;
            ...
            if (!dbPlayer.HasData("carsellTuneColor1") || !dbPlayer.HasData("carsellTuneColor2"))
            {
                dbPlayer.SendNewNotification($"Sie müssen zuerst die Farbe definieren!");
                return;
            }
        ...
        [RemoteEvent]
        public void JobCarsellSetPriceAttach(Player player, string returnstring, string key)
        {
            if (!player.CheckRemoteEventKey(key)) return;
            DbPlayer dbPlayer = player.GetPlayer();
            if (dbPlayer == null || !dbPlayer.IsValid()) return;
        ...
        [RemoteEvent]
        public void JobCarsellCustomerBuy(Player player, string returnstring, string key)
        {
            if (!player.CheckRemoteEventKey(key)) return;
            DbPlayer dbPlayer = player.GetPlayer();
            if (dbPlayer == null || !dbPlayer.IsValid()) return;
            ...
            if (!dbPlayer.HasData("carshop_sellerId") || !dbPlayer.HasData("carshop_vehicleDataId") ||
            ...
            else
            {
                ...
                JobCarsellFactionModule.Instance.InsertCustomerOrder(seller.Id, dbPlayer.Id, seller.TeamId, (int)Data.Id, -1, dbPlayer.GetData("carshop_color1"), dbPlayer.GetData("carshop_color2"));
