...
namespace VMP_CNR.Module.Konversations
{
    public class MessengerOverviewApp : SimpleApp
    {
        ...
        [RemoteEvent]
        public void deletePhoneChat(Player p_Player, string p_ConversationIDobj, string key)
        {
            ...
            if (!Int32.TryParse(p_ConversationIDobj, out int p_ConversationID)) return;
        ...
        [RemoteEvent]
        public void sendPhoneMessage(Player p_Player, string p_ReceiverObj, string p_Message, string key)
        {
            if (!p_Player.CheckRemoteEventKey(key)) return;
            ...
            if (!UInt32.TryParse(p_ReceiverObj, out uint p_Receiver)) return;
            ...
            // if (toomany)
            // {
            //     l_DbPlayer.SendNewNotification("Das Postfach des Empfängers ist voll - SMS nicht gesendet.");
            //     DbPlayer receiver = TelefonInputApp.GetPlayerByPhoneNumber(Convert.ToInt32(p_Receiver));
            //     if (receiver != null || receiver.IsValid())
            //     {
            //         receiver.SendNewNotification("Dein SMS-Postfach ist voll - bitte lösche einige SMS.");
            //     }
            //     return;
            // }
            ...
            TriggerNewClient(p_Player, "updateChat", l_Json);
            ...
            {
                TriggerNewClient(l_Player.Player, "updateChat", l_Json);
            }
        ...
        [RemoteEvent]
        public void responsePhoneMessage(Player p_Player, string p_MsgPartnerObj, string p_Answer, string key)
        {
            if (!p_Player.CheckRemoteEventKey(key)) return;
            DbPlayer l_DbPlayer = p_Player.GetPlayer();
            ...
            if (!UInt32.TryParse(p_MsgPartnerObj, out uint p_MsgPartner)) return;
            ...
            if (!l_Player.phoneSetting.flugmodus)
            {
                ...
                l_Player.Player.TriggerNewClient("playSMSRingtone");
            }
        }
    }
}
