...
namespace VMP_CNR.Module.Konversations
{
    public class MessengerListApp : SimpleApp
    {
        ...
        [RemoteEvent]
        public void requestKonversations(Player p_Player, string key)
        {
            ...
            {                    
                ...
                {
                    foreach (var kvp in KonversationsModule.Instance.konversations.ToList().Where(ks => ks.Key.Player1 == dbPlayer.handy[0] || ks.Key.Player2 == dbPlayer.handy[0]))
                    {
                        if (kvp.Value == null) continue;

                        ClientKonversation clientKonversation = new ClientKonversation();
                        clientKonversation.KonversationId = kvp.Key.Id;
                        clientKonversation.KonversationPartnerName = (kvp.Key.Player1 == dbPlayer.handy[0]) ?
                            (dbPlayer.PhoneContacts.TryGetPhoneContactNameByNumber(kvp.Key.Player2) != null ? dbPlayer.PhoneContacts.TryGetPhoneContactNameByNumber(kvp.Key.Player2) : "" + kvp.Key.Player2) :
                            (dbPlayer.PhoneContacts.TryGetPhoneContactNameByNumber(kvp.Key.Player1) != null ? dbPlayer.PhoneContacts.TryGetPhoneContactNameByNumber(kvp.Key.Player1) : "" + kvp.Key.Player1);

                        clientKonversation.KonversationPartnerNumber = (kvp.Key.Player1 == dbPlayer.handy[0]) ? kvp.Key.Player2 : kvp.Key.Player1;
                        ...
                        foreach (ConvMessage konversationMessage in kvp.Value)
                        {
                            if (konversationMessage == null) continue;
                            string l_Sender = "";
                            if (konversationMessage.SenderId == dbPlayer.handy[0])
                                l_Sender = "Ich";
                            else if (dbPlayer.PhoneContacts.TryGetPhoneContactNameByNumber(konversationMessage.SenderId) != null)
                                l_Sender = dbPlayer.PhoneContacts.TryGetPhoneContactNameByNumber(konversationMessage.SenderId);
                            else
                                l_Sender = "" + konversationMessage.SenderId;
                            ...
                            clientKonversationMessage.MessageSenderName = l_Sender;
                            clientKonversationMessage.Message = l_Message;
                            clientKonversationMessage.Receiver = (dbPlayer.handy[0] == konversationMessage.SenderId);
                            clientKonversation.KonversationMessages.Add(clientKonversationMessage);
                        }
                        clientKonversations.Add(clientKonversation);
                    }
                ...
                TriggerNewClientBig(p_Player, "responseKonversations", l_Json);
            }
            catch(Exception e)
            {
